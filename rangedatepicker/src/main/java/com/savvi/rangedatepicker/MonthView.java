...
public class MonthView extends LinearLayout {
    ...
    public static MonthView create(ViewGroup parent, LayoutInflater inflater,
        ...
        for (int offset = 0; offset < 7; offset++) {
            ...
            if(weekdayNameFormat.format(today.getTime()).matches("Sun")){
                textView.setText("Su");
            }
            else if(weekdayNameFormat.format(today.getTime()).matches("Mon")){
                textView.setText("Mo");
            }
            else if(weekdayNameFormat.format(today.getTime()).matches("Tue")){
                textView.setText("Tu");
            }
            else if(weekdayNameFormat.format(today.getTime()).matches("Wed")){
                textView.setText("We");
            }
            else if(weekdayNameFormat.format(today.getTime()).matches("Thu")){
                textView.setText("Th");
            }
            else if(weekdayNameFormat.format(today.getTime()).matches("Fri")){
                textView.setText("Fr");
            }
            else if(weekdayNameFormat.format(today.getTime()).matches("Sat")){
                textView.setText("Sa");
            }
            else{
                Toast.makeText(headerRow.getContext(), ""+weekdayNameFormat.format(today.getTime()), Toast.LENGTH_SHORT).show();
